% script para dibujar el entorno con el tema oscuro y radios de conectividad
clear;
close all;
...
figPrincipal = figure('Name', 'Entorno Grid Radios', 'Color', [0.15 0.15 0.15]); 
...
% Dibujar circunferencias de conectividad alrededor de cada RIS (Color Lila)
radius = 100;
th = linspace(0, 2*pi, 100);
for i = 1:length(xRIS)
    x_circle = xRIS(i) + radius * cos(th);
    y_circle = yRIS(i) + radius * sin(th);
    % Color lila con un poco de transparencia, ej: [0.7 0.4 0.9 0.6]
    hConn = plot(axPrincipal, x_circle, y_circle, '--', 'Color', [0.7 0.4 0.9 0.6], 'LineWidth', 1.5);
end
...
lgd = legend(axPrincipal, [hAP, hRIS, hUE, hConn], {'AP', 'RIS', 'UE', 'Área de conectividad'}, 'Location', 'best', 'FontSize', 11, 'Box', 'on');
lgd.TextColor = 'w';
...
savefig(figPrincipal, fullfile('positions', 'entorno_grid_oscuro_radios.fig'));
print(figPrincipal, fullfile('positions', 'entorno_grid_oscuro_radios.png'), '-dpng', '-r300');
fprintf('Figura guardada: positions/entorno_grid_oscuro_radios.png\n');
