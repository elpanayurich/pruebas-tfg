function [APpositions, RISpositions, UEpositions, wrapLocations, APpositionsWrapped] = positions_setup(L, K, squareLength, center_grid_factor, setup_seed)
...
%     APpositions = (rand(L,1) + 1i*rand(L,1)) * squareLength;
% 
%     % Calcular el número de RIS basándose en la lógica original del grid
%     num_RIS = (sqrt(L) + 1)^2; 
% 
%     % Posiciones aleatorias de las RIS con distribución uniforme
%     RISpositions = (rand(num_RIS,1) + 1i*rand(num_RIS,1)) * squareLength;
% 
%     % Posiciones aleatorias de los usuarios (UEs) con distribución uniforme
%     UEpositions = (rand(K,1) + 1i*rand(K,1)) * squareLength;
% 
%     % Calcular ubicaciones alternativas de los APs con wraparound (sin cambios)
%     wrapHorizontal = repmat([-squareLength 0 squareLength],[3 1]);
%     wrapVertical = wrapHorizontal';
%     wrapLocations = wrapHorizontal(:)' + 1i*wrapVertical(:)';
%     APpositionsWrapped = repmat(APpositions,[1 length(wrapLocations)]) + repmat(wrapLocations,[L 1]);
