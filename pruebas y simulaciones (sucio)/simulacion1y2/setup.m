function [R_AP_UE,R_AP_RIS1,R_AP_RIS2,R_RIS_UE,pilotIndex,D,HMean_AP_UE, HMean_AP_RIS, HMean_RIS_UE, probLoS_AP_UE, probLoS_RIS_UE, dist_RIS_UE, UEpositions] = setup(L,K,N_AP,N_RIS,tau_p,seed,ASD_varphi,LoS,fc,S, APpositions, RISpositions, UEpositions, wrapLocations, APpositionsWrapped, h_BS, h_UT, h_RIS)
    %% Definir escenario
    ...
    for k = 1:K
      ...
      % dist_RIS_UE(:,k) = sqrt(distanceVertical_RIS_UE^2 + abs(UEposition - RISpositions).^2);
        dist_RIS_UE(:,k) = 1000000000000000000000;  % Simular sin RIS
