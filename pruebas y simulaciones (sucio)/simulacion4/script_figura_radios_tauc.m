% plot_radio_optimo_tauc.m - Radio Óptimo vs tau_c con barras de error en rojo

% 1. Definir los datos
tau_c = [1000, 4000, 7000, 10000, 13000, 17000, 20000];
radioOptimo = [0, 40, 60, 60, 70, 70, 70];

% Definir el margen de error (10 metros para todos los puntos)
errorMargin = 10 * ones(1, length(tau_c));
...
figPrincipal = figure('Name', 'Radio Optimo vs Tamaño de tau_c', 'Color', [0.15 0.15 0.15]); 
axPrincipal = axes(figPrincipal); 
hold(axPrincipal, 'on'); 

% 4. Graficar PRIMERO las barras de error en ROJO (sin línea principal)
errorbar(axPrincipal, tau_c, radioOptimo, errorMargin, ...
    'Color', colorError, ...
    'LineStyle', 'none', ...  
    'LineWidth', 1.2, ...       
    'Marker', 'none', ...
...
plot(axPrincipal, tau_c, radioOptimo, ...
    'Color', colorLinea(1, :), ...
    'LineStyle', '-', ...
    'Marker', 'o', ...
    'LineWidth', 1.5, ...       
...
xlabel(axPrincipal, '\tau_c', 'FontSize', 12, 'FontWeight', 'bold', 'Color', 'w');
...
savefig(figPrincipal, fullfile('figures_clean', 'RadioOptimo_vs_Tauc_ErrorRojo.fig'));
print(figPrincipal, fullfile('figures_clean', 'RadioOptimo_vs_Tauc_ErrorRojo.png'), '-dpng', '-r300');

fprintf('Figura combinada guardada en figures_clean/RadioOptimo_vs_Tauc_ErrorRojo.fig y .png\n');